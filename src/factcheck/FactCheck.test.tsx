...
describe('FactCheck UI', () => {
  ...
  it('shows loading state, stage updates, and renders a final result', async () => {
    await mockChromeStorage.sync.set({
      factCheckConfigs: [
        { provider: 'OpenAI', apiKey: 'fake', model: 'gpt-4o-mini', language: 'en' },
      ],
      factCheckTimeoutSec: 9,
    });

    const onFactCheck = vi.fn((text: string, onStage?: (stage: string, isRetry?: boolean) => void) => {
    ...
    render(<FactCheck text="Some test text" enabled={true} onFactCheck={onFactCheck} />);
    ...
    await waitFor(() => expect(Number(progress.getAttribute('aria-valuenow'))).toBeGreaterThan(0));
    await waitFor(() => expect(screen.getByText(/Contacting/i)).toBeInTheDocument());
    await waitFor(() => expect(screen.getByText(/Credible/i)).toBeInTheDocument(), { timeout: 1000 });
    expect(screen.getByText(/via OpenAI/i)).toBeInTheDocument();
    expect(onFactCheck).toHaveBeenCalledWith('Some test text', expect.any(Function));
  });
  ...
  it('resets progress and shows retry stage when the provider callback reports a retry', async () => {
    ...
    render(<FactCheck text="Retry test text" enabled={true} onFactCheck={onFactCheck} />);
    ...
    expect(await screen.findByText(/Retrying with another provider…/i)).toBeInTheDocument();
    expect(Number(progress.getAttribute('aria-valuenow'))).toBe(0);

    await waitFor(() => expect(screen.getByText(/Misleading/i)).toBeInTheDocument(), { timeout: 1000 });
  });
});
