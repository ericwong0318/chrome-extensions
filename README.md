...
| **Steps**   | 1. Checkout (`actions/checkout@v4`) 2. Cache npm (`~/.npm`, keyed on `package-lock.json`) 3. `npm ci` 4. `npm test` (Vitest) 5. `npm run build` (Vite → `dist/`) 6. Upload `dist/` as the `extension-dist` artifact |
...
| **Runner**      | `ubuntu-latest` (host) running the `node:24-bookworm-slim` container                                                                                                                                                                                                             |
| **Steps**       | 1. Checkout 2. Cache npm 3. `npm ci` 4. `npm test` 5. `npm run build` 6. `apt-get install zip` (slim image has no `zip`) 7. Zip `dist/` → `zhihu-user-blocker-<tag>.zip` 8. Create GitHub Release with the zip attached (`softprops/action-gh-release@v2`, auto-generated notes) |
